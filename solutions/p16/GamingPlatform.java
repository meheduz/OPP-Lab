...
interface Rankable { void updateRank(); }
...
abstract class User {
    ...
    public User(String userId, String username) { this(userId, username, ""); }
...
abstract class GamePlayer extends User {
    ...
    public GamePlayer(String userId, String username, String email, int gamesPlayed, int wins) {
        ...
        this.gamesPlayed = gamesPlayed; this.wins = wins;
        totalUsers++;
    }
    public int getGamesPlayed() { return gamesPlayed; }
    public int getWins() { return wins; }
    public double getWinRate() { return gamesPlayed == 0 ? 0 : (wins * 100.0 / gamesPlayed); }
...
class RankedPlayer extends GamePlayer implements Rankable {
    ...
    public RankedPlayer(String userId, String username, String email, int gamesPlayed, int wins,
                        String rankTier, int rankPoints) {
        super(userId, username, email, gamesPlayed, wins);
        this.rankTier = rankTier; this.rankPoints = rankPoints; this.seasonWins = 0;
    }
    public String getRankTier() { return rankTier; }
    public int getRankPoints() { return rankPoints; }
    protected void setRankTier(String t) { rankTier = t; }
    protected void addRankPoints(int p) { rankPoints += p; }

    public int calculateXP(boolean isWin) { return isWin ? 200 : 50; }
    public int calculateXP(boolean isWin, int kills) { return calculateXP(isWin) + kills * 10; }

    public void updateRank() {
        addRankPoints(200);
        String newTier;
        if (rankPoints >= 4000) newTier = "LEGENDARY";
        else if (rankPoints >= 3000) newTier = "DIAMOND";
        else if (rankPoints >= 1700) newTier = "PLATINUM";
        else if (rankPoints >= 1000) newTier = "GOLD";
        else newTier = "SILVER";
        setRankTier(newTier);
        System.out.println("Rank Updated: " + getUsername() + " is now " + newTier + " (" + rankPoints + " pts)");
    }
...
class ProPlayer extends RankedPlayer {
    ...
    public ProPlayer(String userId, String username, String email, int gamesPlayed, int wins,
                     String rankTier, int rankPoints, String sponsorName, int tournamentWins, double prizeMoneyEarned) {
        super(userId, username, email, gamesPlayed, wins, rankTier, rankPoints);
        this.sponsorName = sponsorName; this.tournamentWins = tournamentWins; this.prizeMoneyEarned = prizeMoneyEarned;
    }

    public int calculateXP(boolean isWin) { return super.calculateXP(isWin) + 100; }
    public int calculateXP(boolean isWin, int kills) { return calculateXP(isWin) + kills * 10; }

    public void updateRank() {
        addRankPoints(300);
        if (tournamentWins >= 10) setRankTier("LEGENDARY");
...
class GameTitle {
    ...
    public GameTitle(String gameId, String gameName, String genre) {
        this.gameId = gameId; this.gameName = gameName; this.genre = genre;
    }
...
public class GamingPlatform {
    public static void main(String[] args) {
        ...
        RankedPlayer rp = new RankedPlayer("U001", "ShadowX", "shadow@arena.com",
                120, 80, "Gold", 1500);

        ProPlayer pp = new ProPlayer("U002", "NightHawk", "hawk@arena.com",
                500, 420, "Diamond", 4200, "RedBull", 12, 850000.0);
